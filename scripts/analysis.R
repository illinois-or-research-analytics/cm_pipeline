...
if (length(args)==0) {
  stop("At least three arguments must be supplied: parent_network_cleaned.tsv","user-specified-analyis.csv", "<file_1.tsv" call.=FALSE)
...
t <- cbind(names(t),rbindlist(t))[,.(Rx=V1,clus_ct=cc,node_cov=nc,node_count=count,min,med,max)]
write.table(t, file=args[2], sep=",", col.names=F, row.names=F)
