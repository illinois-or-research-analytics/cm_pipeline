...
@click.option(
    ...
    required=True,
    type=click.Path(),
    help="Working directory for intermeidate files",
)
def run_cm_pipeline(pipeline_file, working_directory):
        ...
        with open(f"{working_directory}/pipeline.json", "w") as fw:
            for line in fr:
                if '"input_file":' in line:
                    original_input_file = line.split('"input_file":')[1].strip()[
                        1:-2
                    ]  # technically not robust because of quotes and commas
                    fw.write(f'\t"input_file": "{converted_input_file_name_only}",\n')
                else:
                    if '"output_dir":' in line:
                        original_output_directory = line.split('"output_dir":')[
                            1
                        ].strip()[1:-2]
                    fw.write(line)
    ...
    sub_run_directories = glob.glob(
        f"{working_directory}/{original_output_directory}/*/*"
    )
    for sub_run_directory in sub_run_directories:
        ...
        if current_path.is_dir() and current_path.name != "analysis":
            ...
            os.system(f"mkdir -p ./{original_output_directory}/{current_path.name}")
            tsv_files = glob.glob(
                f"./{working_directory}/{original_output_directory}/*/{current_path.name}/*.tsv"
            )
            max_stage_file = ""
            max_stage_num = -1
