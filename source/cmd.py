...
class Cmd(object):
    def __init__(self, default_config_obj):
        ...
        self.executed_cmds_folder = os.path.join(ROOT_DIR, '..','logs','executed-cmds')
        os.makedirs(self.executed_cmds_folder, exist_ok=True)
        self.executed_cmds_text = os.path.join(self.executed_cmds_folder, f'ex_cmds-{self.default_config.timestamp}.txt')
    ...
    def write_cmds_to_file(self, cmd):
        with open(self.executed_cmds_text, 'a+') as fp:
            cmd_pl_h1 = cmd.replace(self.default_config.output_dir, '/placeholder1')
            cmd_pl_h2 = cmd_pl_h1.replace(self.default_config.root_output_dir, '/placeholder2')
            fp.write(cmd_pl_h2)
            fp.write("\n\n")
