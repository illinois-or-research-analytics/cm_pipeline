...
class Filtering(Stage):
    ...
    def initialize(self, data):
        ...
        for param in self.params:
            ...
            self.output_file.append(f'{self.working_dir}/\
                                    {folder_name}/\
                                        S{self.index}_{self.network_name}_{self.algorithm}.\
                                            {suffix}{filtering_operation}.tsv')
    ...
    def get_stage_commands(self, project_root, prev_file):
        ...
        for j, (param, output_file) in enumerate(zip(self.params, self.output_file)):
            ...
            for i, script in enumerate(self.scripts):
                ...
                curr_output_file = f'{folder_name}/\
                    S{self.index}_{self.network_name}_{self.algorithm}.\
                        {suffix}{filtering_operation}.tsv'
